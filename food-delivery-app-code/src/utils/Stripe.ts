...
export class Stripe {
    ...
    private static _stripe = new stripe(getEnvironmentVariables().stripe.secret_key, {apiVersion: '2022-11-15'});
    ...
    static async createCustomer(name: string, email: string) {
        try {
            ...
            const customer: stripe.Customer = await Stripe._stripe.customers.create(params);
